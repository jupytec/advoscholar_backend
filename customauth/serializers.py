...
class UserProfileSerializer(serializers.ModelSerializer):
    class Meta:
        model = UserProfile
        fields = (
            'first_name', 'last_name', 'name', 'about', 'profile_photo',
            'short_description', 'instagram_link', 'facebook_link',
            'twitter_link')
...
class UserProfileUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            'instagram_link', 'facebook_link', 'twitter_link', 'profile_photo',
            'current_password', 'new_password', 'new_password2', 'name')
    ...
    def validate(self, data):
        ...
        if current_password and not \
                self.instance.user.check_password(current_password):
            raise serializers.ValidationError(
        ...
        if new_password:
            password_validation.validate_password(
                new_password, self.instance.user)
    ...
    def update(self, instance, validated_data):
        ...
        # Combine first and last names or
        # set to an empty string if both are None
        instance.name = (
