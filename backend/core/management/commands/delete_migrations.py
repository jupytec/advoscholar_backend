...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'app_name',
            type=str,
            help='Name of the app to delete migrations from')
        ...
        if os.path.exists(migration_path):
            ...
            if migration_files:
                for file_name in migration_files:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f'Deleted migration file: {file_name}'))
            else:
                self.stdout.write(
                    self.style.SUCCESS(
                        f'No migration files to delete in {app.label}'
                        )
                )
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    f'No migrations directory found in {app.label}'
                    )
                )
